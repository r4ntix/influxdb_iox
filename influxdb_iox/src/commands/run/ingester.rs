...
use iox_catalog::interface::{RepoCollection, KafkaPartition};
use ingester::handler::{IngestHandler, IngestHandlerImpl};
...
pub async fn command(config: Config) -> Result<()> {
    ...
    let catalog: Arc<dyn RepoCollection> = Arc::new(Arc::new(PostgresCatalog::connect("ingester", iox_catalog::postgres::SCHEMA_NAME, &config.catalog_dsn).await?));

    let kafka_topic = match catalog.kafka_topic().get_by_name(&config.kafka_topic).await? {
        Some(k) => k,
        None => return Err(Error::KafkaTopicNotFound(config.kafka_topic))
    };
    let kafka_partitions: Vec<_> = (config.kafka_partition_range_start..config.kafka_partition_range_end).map(|p| KafkaPartition::new(p)).collect();

    let ingest_handler = Arc::new(IngestHandlerImpl::new(kafka_topic, kafka_partitions, catalog));
